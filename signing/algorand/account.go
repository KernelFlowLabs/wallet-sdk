...
func NewAccountFromPrivateKey(privateKey []byte) (signing.AccountHandler, error) {
	publicKey, err := key.PrivateKey2PublicKeyED25519(privateKey)
	if err != nil {
		return nil, fmt.Errorf("PrivateKey2PublicKeyED25519: %v", err)
	}
...
func (a *Account) PublicKeyHex() string  { return hex.EncodeToString(a.publicKey) }
